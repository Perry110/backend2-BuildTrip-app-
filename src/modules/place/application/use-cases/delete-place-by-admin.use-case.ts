...
export interface DeletePlaceByAdminCommand {
  placeId: string;
  reason: string;
...
export class DeletePlaceByAdminUseCase {
  ...
  async execute(command: DeletePlaceByAdminCommand): Promise<void> {
    ...
    place.deleteByAdmin(command.actor, command.reason);
